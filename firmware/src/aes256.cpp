#include "aes256.h"

static const uint8_t sbox[256] = {
    0x63, 0x7c, 0x77, 0x7b, 0xf2, 0x6b, 0x6f, 0xc5, 0x30, 0x01, 0x67, 0x2b, 0xfe, 0xd7, 0xab, 0x76,
    0xca, 0x82, 0xc9, 0x7d, 0xfa, 0x59, 0x47, 0xf0, 0xad, 0xd4, 0xa2, 0xaf, 0x9c, 0xa4, 0x72, 0xc0,
    0xb7, 0xfd, 0x93, 0x26, 0x36, 0x3f, 0xf7, 0xcc, 0x34, 0xa5, 0xe5, 0xf1, 0x71, 0xd8, 0x31, 0x15,
    0x04, 0xc7, 0x23, 0xc3, 0x18, 0x96, 0x05, 0x9a, 0x07, 0x12, 0x80, 0xe2, 0xeb, 0x27, 0xb2, 0x75,
    0x09, 0x83, 0x2c, 0x1a, 0x1b, 0x6e, 0x5a, 0xa0, 0x52, 0x3b, 0xd6, 0xb3, 0x29, 0xe3, 0x2f, 0x84,
    0x53, 0xd1, 0x00, 0xed, 0x20, 0xfc, 0xb1, 0x5b, 0x6a, 0xcb, 0xbe, 0x39, 0x4a, 0x4c, 0x58, 0xcf,
    0xd0, 0xef, 0xaa, 0xfb, 0x43, 0x4d, 0x33, 0x85, 0x45, 0xf9, 0x02, 0x7f, 0x50, 0x3c, 0x9f, a8,
    
};

static uint8_t gfm2(uint8_t x) { return (x << 1) ^ (x & 0x80 ? 0x1b : 0); }

void aes256_encrypt(uint8_t* state, const uint8_t* key) {
    uint8_t round_key[240];
    memcpy(round_key, key, 32);
    
    // Key Expansion for 256-bit (14 rounds -> 15 round keys of 16 bytes = 240 bytes)
    uint8_t rcon = 1;
    for (int i = 32; i < 240; i += 4) {
        uint8_t temp[4];
        memcpy(temp, &round_key[i - 4], 4);
        if (i % 32 == 0) {
            uint8_t t = temp[0];
            temp[0] = sbox[temp[1]] ^ rcon;
            temp[1] = sbox[temp[2]];
            temp[2] = sbox[temp[3]];
            temp[3] = sbox[t];
            rcon = gfm2(rcon);
        } else if (i % 32 == 16) {
            for (int k = 0; k < 4; k++) temp[k] = sbox[temp[k]];
        }
        for (int j = 0; j < 4; j++) round_key[i + j] = round_key[i - 32 + j] ^ temp[j];
    }

    // Initial Round
    for (int i = 0; i < 16; i++) state[i] ^= round_key[i];

    // 13 Rounds
    for (int round = 1; round < 14; round++) {
        for (int i = 0; i < 16; i++) state[i] = sbox[state[i]];
        // ShiftRows
        uint8_t t = state[1]; state[1] = state[5]; state[5] = state[9]; state[9] = state[13]; state[13] = t;
        t = state[2]; state[2] = state[10]; state[10] = t; t = state[6]; state[6] = state[14]; state[14] = t;
        t = state[15]; state[15] = state[11]; state[11] = state[7]; state[7] = state[3]; state[3] = t;
        // MixColumns
        for (int i = 0; i < 16; i += 4) {
            uint8_t a = state[i], b = state[i+1], c = state[i+2], d = state[i+3];
            state[i]   = gfm2(a ^ b) ^ b ^ c ^ d;
            state[i+1] = gfm2(b ^ c) ^ c ^ d ^ a;
            state[i+2] = gfm2(c ^ d) ^ d ^ a ^ b;
            state[i+3] = gfm2(d ^ a) ^ a ^ b ^ c;
        }
        for (int i = 0; i < 16; i++) state[i] ^= round_key[round * 16 + i];
    }

    // Final Round
    for (int i = 0; i < 16; i++) state[i] = sbox[state[i]];
    uint8_t t = state[1]; state[1] = state[5]; state[5] = state[9]; state[9] = state[13]; state[13] = t;
    t = state[2]; state[2] = state[10]; state[10] = t; t = state[6]; state[6] = state[14]; state[14] = t;
    t = state[15]; state[15] = state[11]; state[11] = state[7]; state[7] = state[3]; state[3] = t;
    for (int i = 0; i < 16; i++) state[i] ^= round_key[224 + i];
}
